...
bash ~/DanxyZsh/T-DanxyZsh.sh ${col} ${TNAME}
echo "";
...
tput sgr 0
if [[ ${PROC32} == [Y/y] ]]; then
	if [ -e $HOME/DanxyZsh.txt ]; then
		rm $HOME/DanxyZsh.txt;
	fi

	if [ -d $HOME/DanxyZsh ]; then
	cd $HOME/DanxyZsh
	fi
...
clear;
echo -e "\033[31m$(figlet -f Remo773 "DanxyZsh")\e[0m"
echo -e "\e[1;32m
+----------------------------------*/
DanxyZsh: (\e[33m80.38.2025\e[32m)
